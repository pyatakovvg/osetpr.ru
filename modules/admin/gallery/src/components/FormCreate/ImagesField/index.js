  ...
  return (
    <div className={styles['wrapper']}>
      <div className={styles['controls']}>
        <Button mode={Mode.PRIMARY} size={Size.SMALL} onClick={handleAddImages}>Загрузить</Button>
      </div>
      <div className={styles['content']}>
        {(input['value'] || []).map((file, index) => (
          <ImageView key={index} index={index} src={URL.createObjectURL(file)} onRemove={() => handleRemove(index)} />
        ))}
      </div>
    </div>
  );
