...
domains=("osetpr.ru" "order.osetpr.ru" "admin.osetpr.ru")
...
for domain in ${!domains[*]}; do
  ...
  if [ ! -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    docker-compose --file "../compose.prod.yml" run --rm --entrypoint "openssl req -x509 -nodes -newkey rsa:1024 \
...
docker-compose --file "../compose.prod.yml" up -d proxy && docker-compose --file "../compose.prod.yml" restart proxy
...
for domain in ${!domains[*]}; do
  ...
  if [ -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    docker-compose --file "../compose.prod.yml" run --rm --entrypoint "certbot certonly --webroot -w /var/www/certbot --cert-name $domain_name $domain_args \
